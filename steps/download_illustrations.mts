...
async function downloadIllustration(sentence: ScriptSentence, index: number, outputFolder: string) {
  if (!sentence.illustrationVideo) {
    throw new Error(`Illustration video url must be set for sentence ${index + 1}`);
  }
